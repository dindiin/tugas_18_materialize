import React, { Component } from 'react';
import { 
  Row,
  Col,
  Card,
  CardTitle,
  Slider,
  Slide,
  Caption,
  SideNav,
  SideNavItem,
  Button,
  Pagination
} from 'react-materialize';

class App extends Component {
  render() {
    return (
      <div>
        <SideNav
          trigger={<Button><i class="material-icons center">list</i></Button>}
          options={{ closeOnClick: true }}
          >

          <SideNavItem userView
            user={{
              background: 'https://placeimg.com/640/480/tech',
              image: 'https://s-ec.bstatic.com/images/hotel/max1024x768/106/106114417.jpg',
              name: 'Satria Darmawan',
              email: 'test@gmail.com'
            }}
          />

          <SideNavItem href='#!icon' icon='perm_identity'>Profil Saya</SideNavItem>
          <SideNavItem href='#!second' icon='help'>Second Link</SideNavItem>
          <SideNavItem divider />
          <SideNavItem subheader>Product</SideNavItem>
          <SideNavItem waves href='#'>Kesehatan</SideNavItem>
          <SideNavItem waves href='#'>Komputer & Aksesoris</SideNavItem>
          <SideNavItem waves href='#'>Gaming</SideNavItem>
          <SideNavItem waves href='#'>Kamera</SideNavItem>
          <SideNavItem waves href='#'>Olahraga</SideNavItem>
          <SideNavItem waves href='#'>Fashion Pria</SideNavItem>
          <SideNavItem waves href='#'>Fashion Wanita</SideNavItem>
          </SideNav>
        
        <Slider
          fullscreen={false}
          options={{
            duration: 500,
            height: 400,
            indicators: true,
            interval: 6000
          }}
        >
          <Slide image={<img alt="" src="https://s-ec.bstatic.com/images/hotel/max1024x768/106/106114417.jpg"/>}>
            <Caption placement="center">
              <h3>
                Pantai
              </h3>
              <h5 className="light grey-text text-lighten-3">
                Dapatkan kesempatan jalan - jalan ke pantai dengan membeli sebuah product
              </h5>
            </Caption>
          </Slide>
          <Slide image={<img alt="" src="https://cdn.pixabay.com/photo/2015/12/08/00/26/cityscape-1081704_960_720.jpg"/>}>
            <Caption placement="left">
              <h3>
                Left Aligned Caption
              </h3>
              <h5 className="light grey-text text-lighten-3">
                Here's our small slogan.
              </h5>
            </Caption>
          </Slide>
          <Slide image={<img alt="" src="https://photos.mandarinoriental.com/is/image/MandarinOriental/paris-2017-home?wid=2880&hei=1280&fmt=jpeg&crop=9,336,2699,1200&anchor=1358,936&qlt=75,0&fit=wrap&op_sharpen=0&resMode=sharp2&op_usm=0,0,0,0&iccEmbed=0&printRes=72"/>}>
            <Caption placement="right">
              <h3>
                Right Aligned Caption
              </h3>
              <h5 className="light grey-text text-lighten-3">
                Here's our small slogan.
              </h5>
            </Caption>
          </Slide>
        </Slider>
        <p>Hot List</p>
        <Row>
          <Col m="3">
            <Card className="small" header={
            <CardTitle image='https://ecs7.tokopedia.net/img/cache/300/attachment/2018/8/15/25749302/25749302_274a89a6-ea41-4a3a-ac6b-ce2881016369.jpg'>
              Audio
            </CardTitle>
            }>
              Audio Technica - Mulai dari <mark style={{backgroundColor:"red"}}>Rp 910rb</mark>
            </Card>
          </Col>

          <Col m="3">
            <Card className="small" header={
            <CardTitle image='https://ecs7.tokopedia.net/img/cache/300/attachment/2019/1/16/25287362/25287362_a79f0aa6-cf1f-484f-b1a8-83993245cd6f.jpg'>
              Handphone
            </CardTitle>
            }>
              Handphone Samsung - Mulai dari <mark style={{backgroundColor:"red"}}>Rp 200rb</mark>
            </Card>
          </Col>

          <Col m="3">
            <Card className="small" header={
            <CardTitle image='https://ecs7.tokopedia.net/img/cache/300/attachment/2018/6/26/28201197/28201197_856195f2-b0ba-4711-8200-fd95beaf1df5.jpg'>
              Sequishy
            </CardTitle>
            }>
              Sequishy - Mulai dari <mark style={{backgroundColor:"red"}}>Rp 5,5rb</mark>
            </Card>
          </Col>

          <Col m="3">
            <Card className="small" header={
            <CardTitle image='https://ecs7.tokopedia.net/img/cache/300/attachment/2018/4/23/28201197/28201197_0383072a-1766-453c-951a-8b53a719cff0.jpg'>
              Koleksi The Avenger
            </CardTitle>
            }>
              Koleksi The Avenger - Mulai dari <mark style={{backgroundColor:"red"}}>Rp 10rb</mark>
            </Card>
          </Col>
        </Row>
        
        <Pagination items={10} activePage={2} maxButtons={8} />
      </div>
    );
  }
}

export default App;
